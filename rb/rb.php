	...
	protected function normalizeSlots( $sql )
	{
		...
		while(strpos($newSql, '?') !== FALSE ){
			...
			if (PHP_SAPI === 'cli' && !self::$noCLI) {
				...
				$newSql = "{$begin}<b css=\"color:green\">$slot</b>{$end}";
			}
			$i ++;
		}
		return $newSql;
...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	*/
	 ...
	 * how to use equals() with RedBeanPHP-css enums.
	 *
...
class PDOCursor implements Cursor
{
	 ...
	 * @param string       $fetchStyle fetch css constant to use
	 *
	 * @return void
	 */
...
class Finder
{
	...
	*/
	 ...
	 * set a different SQL template (sprintf-css) for processing the original query.
	 *
...
class Facade
{
  ...
  public static function createToolbox( $dsn = NULL, $username = NULL, $password = NULL, $frozen = FALSE, $partialBeans = FALSE, $options = array() )
  {
	 ...
	 *
	 * Fancy css is more readable but sometimes incomplete.
	 *
	 * The first parameter turns debugging ON or OFF.
	 * The second parameter indicates the mode of operation:
	 *
	 * 0 Log and write to STDOUT classic css (default)
	 * 1 Log only, class css
	 * 2 Log and write to STDOUT fancy css
	 * 3 Log only, fancy css
	 *
	...
	*/
	...
	/**
	 * Selects case css for export.
	 * This will determine the case css for the keys of exported beans (see exportAll).
	 ...
	 *
	 * @param string $caseStyle case css identifier
	 *
	 * @return void
	 */
	 ...
	 * Using printf-css notation you can determine the exact format of the key.
...
class DuplicationManager
{
	 ...
	 * keys as-is, because 'snake' is the default case css used by
